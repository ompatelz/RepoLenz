  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextGraph, nextStats] = await Promise.all([
        getJson<GraphDocument>("/api/graph"),
      ...
      <section className="workspace">
        ...
        <section className="content" id="architecture">
          <div className="content-header">
            ...
            <button className="button secondary" onClick={() => setSelectedId(null)} type="button" disabled={!selected}>Clear selection</button>
          </div>
        ...
        <aside className="inspector" aria-label="Node inspector">
          <div className="inspector-header"><span>INSPECTOR</span>{selected && <button className="close-button" onClick={() => setSelectedId(null)} type="button" aria-label="Close inspector">×</button>}</div>
          {!selected ? <div className="inspector-empty"><div className="empty-icon small">◇</div><h2>Nothing selected</h2><p>Select a node in the graph to inspect its source and relationships.</p></div> : <div className="inspector-detail">
